...
/**
 * Base class for content indexes (blocks, data)
 * Maintains collection->name->locale lookups with V8-optimized structures
 */
export class BaseContentIndex {
  // Primary index: id -> ContentManifest
  protected index: Map<string, ContentManifest> = new Map();

  // Content cache: id -> locale -> content (for small files)
  protected contentCache: Map<string, Record<string, ContentData | null>> = new Map();

  // Name index: collection -> name -> ContentManifest
  protected nameIndex: Map<string, Map<string, ContentManifest>> = new Map();
