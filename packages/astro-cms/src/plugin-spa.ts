...
export interface ConlocaCMSOptions extends Omit<UIConfig, 'basename'> {
  ...
  /**
   * @deprecated Use componentPaths instead. Will be removed in next major version.
   * Components that can be hydrated in the browser.
   * Maps component names to their file paths for dynamic import.
   *
   * @example
   * ```typescript
   * hydratableComponents: {
   *   'TestimonialGrid': './src/components/TestimonialGrid.tsx',
   *   'FilteredList': './src/components/FilteredList.tsx',
   * }
   * ```
   */
  hydratableComponents?: Record<string, string>;
}
            ...
            plugins: [
              {
                ...
                async load(id) {
                  ...
                  if (id === RESOLVED_HYDRATION_REGISTRY) {
                    // Prefer scanner results if componentPaths configured
                    if (hydrationDiscoveriesPromise) {
...
${entries.join(',\n')}
                    ...
                    // Deprecated: fall back to hydratableComponents config
                    const components = options.hydratableComponents || {};

                    if (Object.keys(components).length === 0) {
                      return 'export const componentRegistry = {};';
                    }

                    // Generate loaders that extract .render from Puck ComponentConfig at import time
                    const entries = Object.entries(components).map(([name, path]) => {
                      const absolutePath = path.startsWith('.') ? `/${path.slice(2)}` : path;
                      // Import module, get named export, extract render function
                      // Note: deprecated path doesn't include strategy - defaults to 'load'
                      return `  '${name}': {
    loader: () => import('${absolutePath}').then(m => m.${name}.render),
    strategy: 'load',
  }`;
                    });

                    return `// Generated by @conloca/astro-cms (deprecated hydratableComponents)
export const componentRegistry = {
${entries.join(',\n')}
};`;
                  }
