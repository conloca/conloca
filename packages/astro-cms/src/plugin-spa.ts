            ...
            plugins: [
              {
                ...
                load(id) {
                  ...
                  if (id === RESOLVED_HYDRATION_REGISTRY) {
                    // Generate component registry from hydratableComponents option
                    const components = options.hydratableComponents || {};

                    if (Object.keys(components).length === 0) {
                      // Empty registry - no hydratable components configured
                      return 'export const componentRegistry = {};';
                    }

                    // Generate import expressions for each component
                    const entries = Object.entries(components).map(([name, path]) => {
                      ...
                      return `  '${name}': () => import('${absolutePath}')`;
                    });

                    return `// Generated by @conloca/astro-cms
