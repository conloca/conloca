...
export function SectionCard({ to, icon, title, description, count, countLabel, isLoading, testId }: SectionCardProps) {
  return (
    <Link
      to={to}
      data-testid={testId}
    ...
    >
      ...
      <div className="mt-auto pt-4 border-t border-gray-100">
        {isLoading ? (
          <div className="flex items-center gap-2 text-gray-400" data-testid={testId ? `${testId}-loading` : undefined}>
            <Loader2 className="h-4 w-4 animate-spin" />
            <span className="text-sm">Loading...</span>
          </div>
        ) : (
          <p className="text-sm">
            <span className="text-2xl font-semibold text-gray-900" data-testid={testId ? `${testId}-count` : undefined}>
              {count ?? 0}
            </span>
