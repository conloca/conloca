...
/**
 * Behavior toggles for the editor's fenced code-block frame chrome. Hosts
 * register an instance from `schemas.ts` (`codeBlockConfig`) and the
 * `@conloca/astro-cms` schemas-loader pushes it into this registry — same
 * window-state + HMR pattern as `mdxComponents`. Theming travels through
 * CSS tokens (`--conloca-code-*`), not this object.
 *
 * Missing fields default to `true` at the call site so a host that omits the
 * export still gets the full chrome.
 */
export interface CodeBlockConfig {
  /** Render a copy-to-clipboard button in the header bar. */
  showCopyButton?: boolean;
  /** Render the filename pulled from the fence meta (`\`\`\`ts title="x.ts"`). */
  showFilename?: boolean;
  /** Render the language pill on the right of the header bar. */
  showLanguageTag?: boolean;
}

interface SharedCodeBlockConfigState {
  config: CodeBlockConfig;
  subscribers: Set<(config: CodeBlockConfig) => void>;
}

const getCodeBlockConfigState = (): SharedCodeBlockConfigState => {
  if (typeof window !== 'undefined') {
    const w = window as unknown as { __CODE_BLOCK_CONFIG_STATE__?: SharedCodeBlockConfigState };
    if (!w.__CODE_BLOCK_CONFIG_STATE__) {
      w.__CODE_BLOCK_CONFIG_STATE__ = {
        config: {},
        subscribers: new Set(),
      };
    }
    return w.__CODE_BLOCK_CONFIG_STATE__;
  }
  return { config: {}, subscribers: new Set() };
};

/**
 * Replace the registered code-block config. Called by the virtual module
 * that loads the host's `schemas.ts`. Notifies subscribers so the editor
 * picks up HMR updates without remounting.
 */
export function setCodeBlockConfig(config: CodeBlockConfig): void {
  const state = getCodeBlockConfigState();
  state.config = config;
  state.subscribers.forEach((fn) => fn(config));
}

/** Read the current config synchronously. */
export function getCodeBlockConfig(): CodeBlockConfig {
  return getCodeBlockConfigState().config;
}

/**
 * React hook subscribing to code-block config changes. Returns the current
 * config and updates on HMR.
 */
export function useCodeBlockConfig(): CodeBlockConfig {
  const [config, setConfig] = useState(() => getCodeBlockConfigState().config);
