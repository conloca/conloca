...
For authentication, add environment variables to the unit file:

```ini
Environment=GITHUB_CLIENT_ID=YOUR_CLIENT_ID
Environment=GITHUB_CLIENT_SECRET=YOUR_CLIENT_SECRET
Environment=SESSION_SECRET=YOUR_SESSION_SECRET_AT_LEAST_32_CHARS
```
...
- [Docker deployment](/docs/guides/deploy/docker/) -- Alternative container-based deployment
