...
<Aside type="caution">
  **Coming Soon** — The `site` query parameter for site-specific block scoping and merging is on the `dev` branch, not
  yet available in a stable release. Global blocks work today.
</Aside>
...
| site       | query | string | no       | If provided, return merged global + site-specific blocks (Coming Soon) |
...
| site       | query | string | no       | Check availability in site-specific scope (Coming Soon)        |
...
| site       | query | string | no       | Site-first lookup: check site blocks, then global (Coming Soon) |
