...
- In development mode, assets are served with `Cache-Control: no-store` to prevent stale file caching during editing.
...
Delete an asset from the media library. Check usage first with the [usage endpoint](#get-assetsusagepath) to avoid
