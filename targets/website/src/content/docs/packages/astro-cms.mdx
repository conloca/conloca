...
<Aside type="note">
  The `auth` option (`AuthConfig`) for built-in authentication is planned but not yet available in the current release.
  See the [Authentication](/concepts/architecture/#authentication) section for details on the upcoming auth provider
  system.
</Aside>
