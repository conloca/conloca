...
const client = new ContentAPIClient({
  baseUrl: '/__conloca/api', // default value
  fetch: customFetch, // optional custom fetch
});
```

The default `baseUrl` is `'/__conloca/api'`, which matches the standard Conloca API mount path. For cross-origin setups,
provide the full base URL (e.g., `https://cms.example.com/__cms/api`).
