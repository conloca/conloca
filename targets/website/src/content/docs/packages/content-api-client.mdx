...
const client = new ContentAPIClient({
  baseUrl: '/__conloca/api', // default: '' (relative URLs)
  fetch: customFetch, // optional custom fetch
});
```

The default `baseUrl` is an empty string `''` (relative URLs), which works when the CMS is served from the same origin.
For cross-origin setups, provide the full base URL (e.g., `https://cms.example.com/__cms/api`).
